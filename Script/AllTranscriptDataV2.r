...
AllIDs_MAE80_F14Sp21_277 <- AllTranscriptDataV3 %>% mutate(degterm=as.numeric(degterm)) %>% 
  filter((course == "ENGRMAE151") & (degterm <= 202114) & (degterm >= 201414) & (degmajor == "277")) %>%
  arrange(id)
AllMEFinished <- AllTranscriptDataV3 %>% filter(id %in% AllIDs_MAE80_F14Sp21_277$id) %>% arrange(id)
...
####### All AE students
AllIDs_MAE80_F14Sp21_279 <- AllTranscriptDataV3 %>% mutate(degterm=as.numeric(degterm)) %>% 
  filter((course == "ENGRMAE80") & (degterm <= 202114) & (degterm >= 201414) & (degmajor == "279")) %>%
  arrange(id)

AllAEFinished <- AllTranscriptDataV3 %>% filter(id %in% AllIDs_MAE80_F14Sp21_279$id) %>% arrange(id)
...
AllAEFinished_151Data <- AllAEFinished %>% filter(course =="ENGRMAE151") %>% arrange(id)
AllAEFinished_159Data <- AllAEFinished %>% filter(course =="ENGRMAE159") %>% arrange(id)


# all students

AllStudents_189Data <- AllTranscriptDataV3 %>% filter((course == "ENGRMAE189")) %>% arrange(id)
AllStudentsYoI_189Data <- AllTranscriptDataV3 %>% filter((course == "ENGRMAE189") & (degterm <= 202114) & (degterm >= 201414)) %>% 
  arrange(id)
...
AllStudentsYoI_189188Data <- AllTranscriptDataV3 %>% filter((course %in% c("ENGRMAE189","ENGRMAE188")) & 
                                                           (degterm <= 202114) & (degterm >= 201414)) %>% 
  arrange(id)

AllStudents_93Data <- AllTranscriptDataV3 %>% filter(course =="ENGRMAE93") %>% arrange(id)
AllStudentsYoI_93Data <- AllTranscriptDataV3 %>% filter((course =="ENGRMAE93") & (degterm <= 202114) & (degterm >= 201414)) %>% 
  arrange(id)
...
SumMEFinished_189188YearData <- AllMEFinished_189188Data %>% group_by(c_year) %>% 
  summarize(totalunits = sum(units), n=n()) %>% 
  distinct(c_year, .keep_all=TRUE) %>% arrange(c_year)
SumMEFinished_189188YearData

# Add up total units of 93 taken by all ME majors
SumMEFinished_93YearData <- AllMEFinished_93Data %>% group_by(c_year) %>% 
  summarize(totalunits = sum(units), n=n()) %>% 
  distinct(c_year, .keep_all=TRUE) %>% arrange(c_year)
...
# Add up total units of 189+188 taken by all ME majors by year
SumAEFinished_189188YearData <- AllAEFinished_189188Data %>% group_by(c_year) %>% 
  summarize(totalunits = sum(units), n=n()) %>% 
  distinct(c_year, .keep_all=TRUE) %>% arrange(c_year)
SumAEFinished_189188YearData

# Add up total units of 93 taken by all ME majors
SumAEFinished_93YearData <- AllAEFinished_93Data %>% group_by(c_year) %>% 
  summarize(totalunits = sum(units), n=n()) %>% 
  distinct(c_year, .keep_all=TRUE) %>% arrange(c_year)
...
SumAllStudentsYoI_189188YearData <- AllStudentsYoI_189188Data %>% group_by(c_year) %>% 
  summarize(totalunits = sum(units), n=n()) %>% 
  distinct(c_year, .keep_all=TRUE) %>% arrange(c_year)
SumAllStudentsYoI_189188YearData

SumAllStudentsYoI_93TermData <- AllStudentsYoI_93Data %>% group_by(term) %>% 
  summarize(totalunits = sum(units), n=n()) %>% 
  distinct(term, .keep_all=TRUE) %>% arrange(term)
SumAllStudentsYoI_93TermData

SumAllStudentsYoI_93YearData <- AllStudentsYoI_93Data %>% group_by(c_year) %>% 
  summarize(totalunits = sum(units), n=n()) %>% 
  distinct(c_year, .keep_all=TRUE) %>% arrange(c_year)
...
AllYearly189188ProjectData <- SumMEFinished_189188YearData %>% 
  left_join(SumAEFinished_189188YearData, by="c_year") %>%
  left_join(SumAllStudentsYoI_189188YearData, by="c_year") %>%
  rename(year=c_year,ME_Units=totalunits.x,ME_Students=n.x,AE_Units=totalunits.y,AE_Students=n.y,All_Units=totalunits,All_Students=n)
AllYearly189188ProjectData

AllYearly93ProjectData <- SumMEFinished_93YearData %>% 
  left_join(SumAEFinished_93YearData, by="c_year") %>%
  left_join(SumAllStudentsYoI_93YearData, by="c_year") %>%
  rename(year=c_year,ME_Units=totalunits.x,ME_Students=n.x,AE_Units=totalunits.y,AE_Students=n.y,All_Units=totalunits,All_Students=n)
...
JoinMEFinished_TE189188Data <- SumMEFinished_TEDataAdj %>% left_join(SumMEFinished_189188Data,by="id") %>% drop_na() %>%
  mutate(ratio=totalunitsadj/(totalunits.y-3))
...
ggplot(JoinMEFinished_TE189188Data, aes(x=ratio, fill=grad_year.x, color=grad_year.x)) + 
  ...
  # scale_x_continuous(breaks = round(seq(min(SumMEFinished_TEDataAdj$x), max(SumMEFinished_TEDataAdj$x), by = 0.5),1))+
  # scale_x_continuous(breaks=seq(0,34,2)) +
  ggtitle("Ratio of Total TE units by 189 units TE Units Taken by MEs \n (subtracted 11 TE units and 3 189 units)") +
  xlab("Total Units (Adjusted)") + ylab("Number of Unique Students")
